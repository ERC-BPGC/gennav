...
def calculate_cost(from_node, to_node):
    ...
    """
    return from_node.cost + math.sqrt(
        (from_node.x - to_node[0]) ** 2 + (from_node.y - to_node[1]) ** 2
    )
...
class RRTStar(object):
    ...
    def __call__(self, goal_point, scan, start_point=[0, 0], animation=False):
        ...
        for iter in range(self.max_iter):
            ...
            rnd_point = sampler(
                self.sample_area, (self.goal.x, self.goal.y), self.goal_sample_rate
            )
    ...
    def search_best_goal_node(self, scan):
        dist_to_goal_list = [
            math.sqrt((n.x - self.goal.x) ** 2 + (n.y - self.goal.y) ** 2)
        ...
        for goal_ind in goal_inds:
            theta = math.atan2(
                self.node_list[goal_ind].y - self.goal.y,
                self.node_list[goal_ind].x - self.goal.x,
            )

            t_node = Node(
                self.node_list[goal_ind].x + math.cos(theta),
                self.node_list[goal_ind].y + math.sin(theta),
            )
